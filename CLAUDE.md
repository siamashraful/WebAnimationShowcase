...
A backdrop **refraction** is layered on via an SVG `feTurbulence → feDisplacementMap` filter referenced from `backdrop-filter: … url(#liquid-glass)`. This is the one cross-browser-fragile piece: `url()` on `backdrop-filter` is non-interoperable — **Blink renders it as strong waves, WebKit applies it softly, others ignore it.** So there are two filter strengths: `#liquid-glass` (gentle, `scale 6`) is the default everyone gets, tuned so Chrome shimmers rather than waves; `#liquid-glass-strong` (`scale 40`) is swapped in for WebKit *only*, scoped to `html.webkit`, because WebKit applies the gentle one so softly the glass would otherwise look clear. That class is set in JS at the top of the IIFE by a UA test (`AppleWebKit` but not `Chrome`/`Chromium`) — a CSS `@supports` query *cannot* reliably isolate Safari (the `-webkit-named-image` hack fails in current Safari) and Blink-vs-WebKit isn't otherwise detectable in CSS, so this one engine split lives in JS by necessity. Engines without backdrop `url()` support fall back cleanly to blur + bevel. And because WebKit renders the displacement so softly that even the strong filter left the pills reading as bare transparency, the `html.webkit` rule *also* frosts them harder — a thicker `blur(14px)` and a milkier paper tint (22%, raised to 55% when `.scrolled` so ink stays readable), which also floors the opacity so they can't be see-through. So Safari has two independent levers: `#liquid-glass-strong`'s `scale` (the warp) and that frost (blur/tint, the body). When tuning: the default `#liquid-glass` affects Chrome; the `html.webkit` block (strong filter + heavier frost) affects Safari/iOS — they never touch each other.
